...
Tests cover all packages that contain pure business logic — validation rules, cryptographic helpers, TOTP validation, and service/repository behaviour against an in-memory SQLite database. The readline loop itself is not tested programmatically; that is verified manually via the demo script.

### With a local Go toolchain (Go 1.21+)
...
|---|---|
| `internal/validation` | Username, password, and date format rules including boundary cases (e.g. the 72-byte bcrypt input limit) |
| `internal/secure` | bcrypt round-trip, AES-GCM encrypt/decrypt round-trip, tampered-ciphertext rejection, session token hash determinism |
| `internal/totp` | Valid code acceptance within the ±1 period skew window; codes outside that window rejected |
| `internal/auth` | Duplicate username rejection, lockout threshold triggering, 2FA enroll-then-verify sequence — failed verification must leave no secret stored in the DB |
| `internal/session` | Session expiry enforcement by AuthGuard; expired rows cleaned up by the sweep |

### Without a local Go toolchain

Run the tests inside the same Docker build used for the app — no extra setup required:
